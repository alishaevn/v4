...
Below is a simplified version of the React class component from part 1 of this series. Today we'll just be going over how to handle the component declaration and the props. Take a look at the class and then continue below.
...
That part was pretty straightforward. Next, we update our props importing. 
...
Something I'm also a fan of is object destructuring. On line 3 above, I've chosen to destructure my prop of `submitText` instead of importing `props` and having to reference `submitText` later in the component. Although we'll see how to do that too in case that's anyone's preference. We'll also see on lines 3 and 5 that parenthesis are used instead of curly brackets. Functional components don't require a `render` statement and we've just destructured our props as an argument to the function. With that being the case, we can directly return the contents of our function instead of using a return statement inside of curly braces.
...
Practice excellence.
Be blessed. :upside-down-smiling-face:
